...
public class ModifyNumber
{
	...
	static int totalNumbersWithSpecificDifference(int N, int diff)
	{
		...
		while (low <= high) 
		{
			...
			/*
			 * if difference between number and its sum of digit is smaller than given
			 * difference then smallest number will be on left side
			 */
			if (mid - modifyNumber(mid) < diff)
			{
				low = mid + 1;
			}

			/*
			 * if difference between number and its sum of digit is greater than or equal to
			 * given difference then smallest number will be on right side
			 */
			else
			{
				high = mid - 1;
